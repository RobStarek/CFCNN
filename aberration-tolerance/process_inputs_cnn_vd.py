...
INPUTS = ['data/input_for_cnn_star_variable_psf.h5']
#'cnn_output_star.h5'
# OUTPUTS = ['data/cnn_output_star_variable_psf.h5']
OUTPUTS = ['/tmp/cnn_output_star_variable_psf.h5']
...
if __name__ == '__main__':
    ...
    for IN, OUT in zip(INPUTS, OUTPUTS):
        ...
        with h5py.File(IN, 'r') as h5fi, h5py.File(OUT, 'w') as h5fo:
            for key, dset in h5fi.items():
                print(f"--- {key} ---")
                if 'stack_' in key:
                    #stack_ prefix denotes images to be transformed, so ... transform them!
                    images = np.array(h5fi[key])
                    images = images.reshape((*images.shape, 1))
                    images /= np.sum(images, axis=(0)).reshape((*images.shape[:-2],1,1))
                    print(images.shape)
                    transformed = model.predict(images)            
                    h5fo.create_dataset(key, data=transformed)                    
                else:
                    #otherwise copy the dataset content
