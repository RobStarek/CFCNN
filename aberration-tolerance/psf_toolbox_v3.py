...
def render_psf_emitters(
    ...
    Parameters:
        ...
        psf (np.ndarray): PSF array for convolution.
    ...
    OFFSET = offset
    for x, y, i in xyi:
        xup = int(round(1 * x +OFFSET))
        yup = int(round(1 * y +OFFSET))
        if xup < 0 or xup >= w:
            continue
        if yup < 0 or yup >= h:
            continue
...
def scaled_psf_vector(px_size_um: float,
    ...
    **A_kwargs : dict
    ...
    mid = int(hires.shape[0]//2)
    zoom = np.s_[mid-25 : mid+25, mid-25 : mid+25]
    scaled = hires[zoom]
    
    # Normalize intensity
    return scaled / scaled.sum(), dx
